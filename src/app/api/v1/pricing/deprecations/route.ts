...
/** GET /api/v1/pricing/deprecations — models with an announced retirement date. */
...
export async function GET() {
  ...
  const payload = await fetchJson<Record<string, unknown>>(
    `${PRICING_API}/v1/pricing/deprecations`,
    3,
  );
  ...
  if (!payload) {
    return jsonError(
      "service_unavailable",
      "The deprecation feed is temporarily unreachable.",
